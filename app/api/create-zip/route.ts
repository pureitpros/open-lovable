...
export async function POST() {
    try {
      ...
      // Read the zip file and convert to base64 (disable line wrapping so the
      // output is a single unbroken base64 string, safe to embed in a data URL)
      const readResult = await global.activeSandboxProvider.runCommand(
              'base64 -w 0 /tmp/project.zip'
            );
